...
import os
import glob
from config import CHECKPOINT_DIR, CHECKPOINT_INTERVAL, BEST_MODEL_DIR
...
class ModelTrainer:
    ...
    def save_checkpoint(
        self, epoch, metrics, model_name, optimizer_name, scheduler_name, loss_name
    ):
    ...
    def save_best_model(
        self, epoch, val_accuracy, model_name, optimizer_name, scheduler_name, loss_name
    ):
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        print(
            f"Best validation accuracy so far: {self.best_val_accuracy:.4f} (epoch {self.best_epoch})"
        )
    ...
    def train(
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for images, labels in tqdm(train_loader, desc="Training"):
                batch_train_loss, batch_train_accuracy = self.train_batch(
                    images, labels
                )
            ...
            epoch_train_losses.append(
                sum(batch_train_losses[-len(train_loader) :]) / len(train_loader)
            )
            epoch_val_losses.append(
                sum(batch_val_losses[-len(val_loader) :]) / len(val_loader)
            )
            epoch_train_accuracies.append(
                sum(batch_train_accuracies[-len(train_loader) :]) / len(train_loader)
            )
            epoch_val_accuracies.append(
                sum(batch_val_accuracies[-len(val_loader) :]) / len(val_loader)
            )
